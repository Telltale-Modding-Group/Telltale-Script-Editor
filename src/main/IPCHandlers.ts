...
import {createModInfo, EditorFile} from '../shared/types';
import {
...
export const registerIPCHandlers = (window: BrowserWindow) => {
	 ...
	 */
	BuildProjectChannel(source).handle(async ({ projectPath, project }) => {
		log('============== Starting...');
...
RegisterSetDescription(set)`;
		...
		for (const directory of rootLevelDirectories) {
			...
			const process = exec(`ttarchext.exe -b 67 "${path.join(tempPath, ttarchName)}" "${path.join(tempPath, directory.name)}"`, (error, stdout, stderr) => {
				log(stdout);
				log(stderr);
			});
			await new Promise(resolve => process.addListener('close', resolve));
		...
		const zipFile = createWriteStream(path.join(projectPath, 'Builds', zipFileName));
